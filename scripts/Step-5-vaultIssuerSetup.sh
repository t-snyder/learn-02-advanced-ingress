...
kubectl exec -it vault-0 -n vault -- vault write auth/kubernetes/config kubernetes_host="https://$KUBERNETES_PORT_443_TCP_ADDR:443"

# Create an Authentication Role
kubectl exec -it vault-0 -n vault -- vault write auth/kubernetes/role/issuer \
...
kubectl apply -n default -f ${WORKDIR}/kube/issuer-secret.yaml

# Create an env var to capture the secret name
ISSUER_SECRET_REF=$(kubectl get secrets -n default --output=json | jq -r '.items[].metadata | select(.name|startswith("issuer-token-")).name')
